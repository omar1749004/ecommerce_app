  ...
  Widget build(BuildContext context) {
   CheckoutControllerImp contrller = Get.put(CheckoutControllerImp());
    return Scaffold(
      ...
      bottomNavigationBar: CustomCartBottom(text: "Checkout",onPressed: (){contrller.cheackout();},),
     body: GetBuilder<CheckoutControllerImp>(builder: (controller) => HandlingDataView(
      statusRequs: contrller.statusRequs, widget:Container(
      padding: const EdgeInsets.all(20),
      child: ListView(children: [
        Text("Chouse Payment Method", style: TextStyle(
        ...
        InkWell(
          ...
          child: CardPaymentMethod(text: "Cash On Delivery",
           isActive:contrller.paymentMethod =="0" ?true: false)),
          InkWell(
           onTap: () {
             contrller.chousePaymentMethod("1");//card
           },
          child: CardPaymentMethod(text: "Payment Card",
           isActive:contrller.paymentMethod =="1" ?true: false),),
       
        SizedBox(height: 20,),
          Text("Chouse Delivery type", style: TextStyle(
          fontWeight: FontWeight.bold,
          fontSize: 16,
          color: ColorApp.secondColor),),
         SizedBox(height: 10,),
         Row(
          ...
          child: CardDeliveryType(imageNmae: ImageAsset.delivry, title: "Delivery",
            isActive:contrller.deliveryType =="0" ? true:false),),
          SizedBox(width: 10,),
          InkWell(
           onTap: () {
             contrller.chouseDeliveryType("1");// 1 =>recive
           },
          child: CardDeliveryType(imageNmae: ImageAsset.delivreytThru, title: "Drive Thru",
            isActive: contrller.deliveryType =="1" ? true:false),),
         ],),
         SizedBox(height: 20,),
        ...
        Column(
          ...
          InkWell(
            ...
            InkWell(
              ...
              child: CardAdress(title: contrller.dataAdress[index].name,
               body: "${contrller.dataAdress[index].city}  ${contrller.dataAdress[index].street}",
               isActive: contrller.addressid ==contrller.dataAdress[index].addressid ?true:false),
            ),)
        ],)
